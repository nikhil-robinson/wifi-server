...
class SnifferUI(QtWidgets.QMainWindow):
    ...
    def start_sniffing(self):
        ...
        self.packets_textbox.clear()
        dir = os.getcwd()
        command = ['pkexec', 'python3', f'{dir}/sniffer.py', selected_iface, sniff_filter]
        p = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output, error = p.communicate()
        if error:
            QtWidgets.QMessageBox.warning(self, "Error", error.decode())
        else:
            self.packets_textbox.insertPlainText(output.decode())

if __name__ == "__main__":
    script_path = os.path.realpath(__file__)

# Elevate privileges using pkexec
    command = ['pkexec', 'python3', script_path]
